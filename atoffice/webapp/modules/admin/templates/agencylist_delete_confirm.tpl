...
({assign var="page_name" value="代理店値引き削除確認"})
...
<tr>
<th>割引き率</th>
<td>({$agencylist.percent})%</td>
</tr>
<tr>
<th>備考</th>
<td width="250">({$agencylist.info|nl2br})</td>
</tr>
</tbody>
</table>
...
<input type="hidden" name="a" value="do_({$hash_tbl->hash('delete_c_agencylist','do')})" />
<input type="hidden" name="sessid" value="({$PHPSESSID})" />
<input type="hidden" name="c_member_id" value="({$agencylist.c_member_id})" />
<input type="hidden" name="target_c_agencylist_id" value="({$agencylist.agency_id})" />
<p class="textBtn"><input type="submit" class="submit" value="　は　い　" /></p>
</form>

<form action="./" method="post">
<input type="hidden" name="m" value="({$module_name})" />
<input type="hidden" name="a" value="page_({$hash_tbl->hash('agency_list')})" />
