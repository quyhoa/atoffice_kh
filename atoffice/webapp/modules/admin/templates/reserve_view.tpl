...
<center>
    ...
    <form name="search" method="POST" action="./">
        <input type="hidden" name="m" value="({$module_name})" />
        <input type="hidden" name="a" value="page_({$hash_tbl->hash('reserve_view','page')})" />
    ...
    <table border=1 width=800 >
        ...
        <tr>
            ...
            <td><span style='margin:5px;'>({$item.tmp_reserve_datetime})</span></td>
        ...
        <tr>
            ...
            <td colspan=3><span style='margin:5px;'>【部屋利用料：({$item.room_price1})円】＋【備品利用料：({$item.vessel_price1})円】＋【サービス利用料：({$item.service_price1})円】＝【合計請求額：({$item.room_price1+$item.vessel_price1+$item.service_price1})円】</td>
        ...
        <tr>
            ...
            <td colspan=3>
        <center>
            <input type="reset" value="　修　正　" onclick="smRevision(({$item.reserve_id}))">
