...
({assign var="page_name" value="入金待ち予約一覧"})
...
<input type="hidden" name="a" value="page_({$hash_tbl->hash('reserve_list','page')})" />
...
({if $limit==0})checked({/if})>すべて
<input type="radio" name="limit" value="1" 
({if $limit==1})checked({/if})>期限内
<input type="radio" name="limit" value="2" 
({if $limit==2})checked({/if})>期限切れ
</td>
</tr>
</table>
...
({if $item.select})
	({$item.page})
({else})
	<a href="./?m=admin&a=page_reserve_list&hall_list=({$hall_id})&u=({$c_member_id})&limit=({$limit})&index=({$item.index})" >({$item.page})</a>
({/if})
|
({/foreach})
<hr>
...
({foreach from=$reserve_list key=key item=item})
	...
	<td ({if $item.pay_limit <= 1 and $item.pay_limit >=-4})bgcolor=#FFFF99({elseif $item.pay_limit < -4})bgcolor=#FF9999({/if})><span style='margin:5px;'>({$item.pay_limitdate})　（残り({$item.pay_limit})日）</span></td>	<td bgcolor=#FFD9DC width=100><span style='margin:5px;'>入金状態</span></td>
	...
	({else})
		...
		<input type="hidden" name="m" value="({$module_name})" />
		<input type="hidden" name="a" value="page_({$hash_tbl->hash('payment','page')})" />
