...
// POST　data
	...
	}elseif(!empty($_REQUEST['amp;sort1'])){
		$sort1 = $_REQUEST['amp;sort1'];
	}

	if(!empty($_REQUEST['sort2'])){
		$sort2 = $_REQUEST['sort2'];
	}elseif(!empty($_REQUEST['amp;sort2'])){
		$sort2 = $_REQUEST['amp;sort2'];
	}elseif(empty($_REQUEST['sort2']) or empty($_REQUEST['amp;sort2'])){
		$sort2 = 1;
	}

	if(!empty($_REQUEST['sort3'])){
		$sort3 = $_REQUEST['sort3'];
	}elseif(!empty($_REQUEST['amp;sort3'])){
		$sort3 = $_REQUEST['amp;sort3'];
	}

	if(!empty($_REQUEST['amp;index'])){
...
if($reserve_list){
	...
	$code.= "</form>";
		...
		if($value['pay_limitdate']){
			...
			if(isset($value['pay_limit']) && $value['pay_limit']>=0){
				if($value['pay_flag']==0 and $value['cancel_flag']==0 and $value['complete_flag']==0){
