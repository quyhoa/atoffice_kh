...
({********************************************************************})
({if $page=="reserve" or $page=="change_reserve"})
...
({elseif $page=="search" or $page=="reserved_info"})
({*************************************************************************})
...
<tr height="38"><td>
<div class="login_name">ようこそ({$c_member.nickname})さん</div>
...
({if $page=='change_reserve' ||  $page=="reserved_info"})
	<tr><td>
	<script type="text/javascript">
		function PerformInputLinkC2()
		{
			LoadHTML('CalendarC2', 'side/calendar2.php({$url})');
		({if $page=="reserved_info"})
			LoadHTML('reserve_info_side', 'side/reserved_info_side.php({$url})');
		({/if})
		}
	</script>
	<div id="CalendarC2">
