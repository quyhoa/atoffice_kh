...
// 画像リスト
class admin_page_hall_access_preview extends OpenPNE_Action
{
	...
	// 県
	$sql = "select value from c_profile_option where c_profile_option_id = ".$hall_data[0]['address_prefecture'];
	$result = db_get_all($sql);
	$this->set('ken', $result[0]['value']);

	// 地図
	$sql = "select image_filename from a_hall_image where hall_id = $hall_id and image_id = 2";
	$result = db_get_all($sql);
	$this->set('image_data', $result[0]);
