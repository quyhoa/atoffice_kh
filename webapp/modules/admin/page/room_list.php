...
class admin_page_room_list extends OpenPNE_Action
{
	...
	// 有効フラグ変更
	if( isset($_POST['flag_change']) && $_POST['flag_change']==100 and $_POST['room_id']){

		$sql = "update a_room SET flag = 0 where hall_id = $hall_id and room_id = ".$_POST['room_id'];
		db_get_all($sql);

	}elseif(isset($_POST['flag_change']) && $_POST['flag_change']==200 and $_POST['room_id']){
	...
	$result[0]['begin'] = isset($result[0]['begin']) ? $result[0]['begin'] : 0;
	$result[0]['finish'] = isset($result[0]['finish']) ? $result[0]['finish'] : null;
	...
	if(empty($result[0]['begin1']) || empty($result[0]['finish1']) )
	{
	...
	if(empty($result[0]['begin2']) || empty($result[0]['finish2']) )
	{
	...
	if(empty($result[0]['begin3']) || empty($result[0]['finish3']) )
	{
	...
	if(empty($result[0]['begin4']) || empty($result[0]['finish4']) )
	{
