...
class admin_page_rental_stop extends OpenPNE_Action
{
    ...
    function execute ($requests)
    {
        ...
        if(isset($_REQUEST['stop_user_id'])){
            ...
            $rs_user=isset($rs_user[0]) ? $rs_user[0] : null;
        ...
        // 日付
        if (isset($_REQUEST['year']) && isset($_REQUEST['month']) && isset($_REQUEST['day']) && $_REQUEST['year'] and $_REQUEST['month'] and $_REQUEST['day']) {
            $year = $_REQUEST['year'];
                ...
                foreach ($value['komawari'] as $k => $v) {
                    if ($count == 0) {
                        ...
                        if (isset($v['reserved']) && $v['reserved']) {
                            ...
                            $opentime2[$line]['reserve_v_list'] = $reserve_v_list;
                            // サービス
                            $v['reserve_id'] = isset($v['reserve_id']) ? $v['reserve_id'] : null;
                            $sql = "select * from a_reserve_s where reserve_id = " .
                                     $v['reserve_id'] . " and cancel_flag = 0";
                            $reserve_s_list = db_get_all($sql);
                            if ($reserve_s_list) {
