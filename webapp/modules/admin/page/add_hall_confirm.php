...
/*	if($_POST['begin']==""){
	...
	//Add 2016-06-21
	if($_POST['begin1'] > $_POST['finish1']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin2'] > $_POST['finish2']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin3'] > $_POST['finish3']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin4'] > $_POST['finish4']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often1'] >= $_POST['finish_often1']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often1'] < $_POST['begin1']){
		array_push($errors, 'error time!');
	}
	if($_POST['finish_often1'] > $_POST['finish1']){
		array_push($errors, 'error time!');
	}
	
	if($_POST['begin_often2'] >= $_POST['finish_often2']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often2'] < $_POST['begin2']){
		array_push($errors, 'error time!');
	}
	if($_POST['finish_often2'] > $_POST['finish2']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often3'] >= $_POST['finish_often3']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often3'] < $_POST['begin3']){
		array_push($errors, 'error time!');
	}
	if($_POST['finish_often3'] > $_POST['finish3']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often4'] >= $_POST['finish_often4']){
		array_push($errors, 'error time!');
	}
	if($_POST['begin_often4'] < $_POST['begin4']){
		array_push($errors, 'error time!');
	}
	if($_POST['finish_often4'] > $_POST['finish4']){
		array_push($errors, 'error time!');
	}
	// if(!array($_POST['usedate'])){
	if(isset($_POST['usedate']) && !array($_POST['usedate'])){
	...
	$usedate = isset($_POST['usedate']) ? array($_POST['usedate']) : null;
	/*$array_check = split(",", $usedate);*/
	if(isset($_POST['usedate']) && $_POST['usedate']){
		$arVal = array();
	...
	$this->set('prefecture', isset($result[0]['value']) ? $result[0]['value'] : null);
