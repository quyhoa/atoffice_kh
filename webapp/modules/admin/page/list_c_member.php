...
class admin_page_list_c_member extends OpenPNE_Action
{
	...
	$this->set('atoffice_auth_type', $result[0]['atoffice_auth_type']);
        ...
        $cond = isset($_REQUEST['cond']) ? substr($_REQUEST['cond'], 1) : null;//add by quyhoa
	...
	$v['profile_list'] = $check_v;
        ...
        // if (isset($cond_list['e_rank']) && isset($cond_list['s_rank']) && !is_null($cond_list['s_rank']) && !is_null($cond_list['e_rank']) && ($cond_list['s_rank'] > $cond_list['e_rank']) ) {//add by quyhoa
        if (!empty($cond_list['s_rank']) && !empty($cond_list['e_rank']) && ($cond_list['s_rank'] > $cond_list['e_rank']) ) {//add by quyhoa
            $msg = '※開始ランクは終了ランクより小さくして下さい';
        }

        if ( isset($msg) && $msg ) {//add by quyhoa
            $v['msg'] = $msg;
        }
