...
class admin_page_set_reserve extends OpenPNE_Action
{
    ...
    function execute ($requests)
    {
        ...
        // 日付
        if (isset($_REQUEST['year']) && isset($_REQUEST['month']) && isset($_REQUEST['day']) && ! is_null($_REQUEST['year']) and ! is_null($_REQUEST['month']) and
        ...
        if (isset($_REQUEST['old_member']) && $_REQUEST['old_member']) {
            $old_member = $_REQUEST['old_member'];
        } else {
            $old_member = '';
        }
        if (isset($_REQUEST['old_hall']) && $_REQUEST['old_hall']) {
                ...
                foreach ($value['komawari'] as $k => $v) {
                    if ($count == 0) {
                        ...
                        if (isset($v['reserved']) && $v['reserved']) {
                            ...
                            // サービス
                            $v['reserve_id'] = isset($v['reserve_id']) ? $v['reserve_id'] : '';
                            $sql = "select * from a_reserve_s where reserve_id = " .
                                     $v['reserve_id'] . " and cancel_flag = 0";
                            $reserve_s_list = db_get_all($sql);
                            if ($reserve_s_list) {
   ...
   // ------------------------------------------------------ period mode
    ...
    foreach ($month_data as $day_data) {
        if(!empty($day_data['no_date_time']))
        {
            ...
            $period_data[$num_date]['cs'] = isset($room_data['cs']) ? $room_data['cs'] : null;
    ...
    }//end foreach month_data
foreach ($period_data as $key => $value) {
$opentime2 = array();
$line = 0;
$v['room_id'] = isset($v['room_id']) ? $v['room_id'] : '';
$period_data[$key]['usedate'] = checkusedate($hall_id, $v['room_id'], $value['year'], $value['month'], 
                        $value['day'],$usedate);

$checkTime = checkDayHoliday( $value['year'], $value['month'], 
                        $value['day']);
if($checkTime == 1){

    if($hall_data['begin_often1'] !='' &&  $hall_data['finish_often1'] != '')
    {
    	$hall_data['begin_often']= $hall_data['begin_often1'];
    	$hall_data['finish_often'] = $hall_data['finish_often1'];
    ...
    if($hall_data['begin_often2'] !='' &&  $hall_data['finish_often2'] != '')
    {
    	$hall_data['begin_often']= $hall_data['begin_often2'];
    	$hall_data['finish_often'] = $hall_data['finish_often2'];
    }
    ...
    if($hall_data['begin_often3'] !='' &&  $hall_data['finish_often3'] != '' )
    {
    	$hall_data['begin_often']= $hall_data['begin_often3'];
    	$hall_data['finish_often'] = $hall_data['finish_often3'];
    }
    ...
    if($hall_data['begin_often4'] !='' &&  $hall_data['finish_often4'] != '')
    {
    	$hall_data['begin_often']= $hall_data['begin_often4'];
    	$hall_data['finish_often'] = $hall_data['finish_often4'];
    }
...
if ($room_data['type'] == 2) {
    ...
    if ($value['opentime']) {
        foreach ($value['opentime'] as $k => $v) {
        	$beginTime = $value['year']."-".sprintf("%02d", $value['month'])."-".sprintf("%02d", $value['day'])." ".$v['begin_time'];
        	$beginOften = $value['year']."-".sprintf("%02d", $value['month'])."-".sprintf("%02d", $value['day'])." ".sprintf("%02d", $hall_data['begin_often']).":00";
        	$finishTime = $value['year']."-".sprintf("%02d", $value['month'])."-".sprintf("%02d", $value['day'])." ".$v['finish_time'];
        	$finishOften = $value['year']."-".sprintf("%02d", $value['month'])."-".sprintf("%02d", $value['day'])." ".sprintf("%02d", $hall_data['finish_often']).":00";
        	if(strtotime($beginTime) < strtotime($beginOften) || strtotime($finishTime) > strtotime($finishOften))
            {
                $v['style'] = 1;
            }
            else{
    ...
    if ($value['komawari']) {
        ...
        foreach ($value['komawari'] as $k => $v) {
            if ($count == 0) {
                ...
                if (isset($v['reserved']) && $v['reserved']) {
                    foreach ($value['komawari'] as $kk => $vv) {
                        if (isset($v['reserved']) && isset($vv['reserved']) && $v['reserved'] == $vv['reserved']) {
