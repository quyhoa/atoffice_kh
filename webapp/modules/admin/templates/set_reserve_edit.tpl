...
<script type="text/javascript">
    ...
    function setRoomID(room_id) 
	{
        ...
        document.forms["reserve"].elements["a"].value = "page_({$hash_tbl->hash('set_reserve_edit','page')})";
        return true;
    }
...
<center>
        ...
        <tr>
            ...
            <td colspan=2>
                <form name="reserve" method="POST" action="./">
                    ...
                    <input type="hidden" name="hall_list" value="({$hall_id})">
                    <input type="text" name="people" value="({$people})"> 人 
                    </td>
                    </tr>
                    <tr>
                        <td>利用目的</td>
                        <td colspan=2>
                            <select name="purpose">
                                <option value="0" ({if $purpose==0})selected="selected"({/if})>--未選択--</option>
                                <option value="1" ({if $purpose==1})selected="selected"({/if})>会議</option>
                                <option value="2" ({if $purpose==2})selected="selected"({/if})>セミナー</option>
                                <option value="3" ({if $purpose==3})selected="selected"({/if})>研修</option>
                                <option value="4" ({if $purpose==4})selected="selected"({/if})>面接・試験</option>
                                <option value="5" ({if $purpose==5})selected="selected"({/if})>懇談会・パーティ</option>
                                <option value="6" ({if $purpose==6})selected="selected"({/if})>その他</option>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>顧客ID番号</td>
                        <td colspan=2>
                            <input type="text" name="c_member_id" value="({$member})"><br>
                            ※ 新規の場合は未入力
                        </td>
                    </tr>
                    ({if !$periodmode})
    ...
    <table width=100%>
        ...
        <td style='border: 1px #000000 solid;text-align: center;vertical-align:middle;' colspan=({$v.cs}) 
            ...
            ({$v.stoped.memo})<br>
            ({assign var=start value=$v.begin_time|substr:0:2})
            ({assign var=end value=$v.finish_time|substr:0:2})
            予約：<input  ({if ($value.room_id==$room_edit && $start>=$start_time && $end <= $end_time)}) checked="checked" ({/if}) type='checkbox' name='reserve_data({$key})_({$k})' id='sd({$key})_({$cb})' value='({$hall_data.hall_id}),({$room_id}),({$value.year})-({$value.month})-({$value.day}) ({$v.begin_time}),({$value.year})-({$value.month})-({$value.day}) ({$v.finish_time}),({$room_data.type})'>
            ...
            ({assign var=start value=$v.begin_time|substr:0:2})
            ({assign var=end value=$v.finish_time|substr:0:2})
            <input ({if ($value.room_id==$room_edit && $start>=$start_time && $end <= $end_time)}) checked="checked" ({/if}) type='checkbox' name='reserve_data({$key})_({$k})' id='sd({$key})_({$cb})' value='({$hall_data.hall_id}),({$room_id}),({$value.year})-({$value.month})-({$value.day}) ({$v.begin_time}),({$value.year})-({$value.month})-({$value.day}) ({$v.finish_time}),({$room_data.type})'>
            ({assign var=cb value=$cb+1})
            ({/if})
        </td>
        ({/foreach})
        ...
        <td style='border: 1px #000000 solid;text-align: center;vertical-align:middle;' colspan=({$v.cs}) 
            ...
            ({assign var=start value=$v.begin_time|substr:0:2})
            ({assign var=end value=$v.finish_time|substr:0:2})
            <input ({if ( $year_edit==$value.year && $month_edit==$value.month && $day_edit==$value.day && $room_id==$room_edit && $start>=$start_time && $end <= $end_time)}) checked="checked" ({/if}) type='checkbox' name='reserve_data({$key})_({$k})' id='sd({$key})_({$cb})' value='({$hall_data.hall_id}),({$room_id}),({$value.year})-({$value.month})-({$value.day}) ({$v.begin_time}),({$value.year})-({$value.month})-({$value.day}) ({$v.finish_time}),({$room_data.type})'>
            ({assign var=cb value=$cb+1})
            ({elseif $v.rest})
            bgcolor=#CDCDCD>休憩
            ({elseif $v.checked})
        ...
        <td style='border: 1px #000000 solid;text-align: center;vertical-align:middle;' colspan=({$v.cs}) 
            ...
            ({assign var=start value=$v.begin_time|substr:0:2})
            ({assign var=end value=$v.finish_time|substr:0:2})
            予約：<input ({if ($value.room_id==$room_edit && $start>=$start_time && $end <= $end_time)}) checked="checked" ({/if}) type='checkbox' name='reserve_data({$key})_({$k})' id='sd({$key})_({$cb})' value='({$hall_data.hall_id}),({$value.room_id}),({$year})-({$month})-({$day}) ({$v.begin_time}),({$year})-({$month})-({$day}) ({$v.finish_time}),({$value.type})'>
            ({assign var=cb value=$cb+1})
            ({elseif $v.checked})
            bgcolor=#FFCCFF>予約
            ({else})
            ...
            ({assign var=start value=$v.begin_time|substr:0:2})
            ({assign var=end value=$v.finish_time|substr:0:2})
            <input ({if ($value.room_id==$room_edit && $start>=$start_time && $end <= $end_time)}) checked="checked" ({/if}) type='checkbox' name='reserve_data({$key})_({$k})' id='sd({$key})_({$cb})' value='({$hall_data.hall_id}),({$value.room_id}),({$year})-({$month})-({$day}) ({$v.begin_time}),({$year})-({$month})-({$day}) ({$v.finish_time}),({$value.type})'>
            ({assign var=cb value=$cb+1})
            ({/if})
        </td>
        ({/foreach})
