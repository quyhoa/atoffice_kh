...
({assign var="page_name" value="顧客リスト"})
...
<table class="userListTable">
	<thead>
		...
		<tr class="min_width">
			...
			<option ({if $cond_list.profile[$prof.name]==$item.c_profile_option_id})selected({/if}) value="({$item.c_profile_option_id})"({if  $c_member.profile[$prof.name].value == $item.value}) selected="selected"({/if})>({$item.value|default:"--"})</option>
			({/foreach})
			</select>
			({/if})
