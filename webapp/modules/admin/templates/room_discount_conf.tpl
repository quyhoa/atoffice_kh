...
<select name="koma1_({$item.num})">
({foreach from=$koma_list item=koma})
<option value="({$koma})" ({if $koma==$item.data.koma1})selected({/if})>({$koma})</option>
({/foreach})
</select>

～

<select name="koma2_({$item.num})">
({foreach from=$koma_list item=koma})
<option value="({$koma})" ({if $koma==$item.data.koma2})selected({/if})>({$koma})</option>
({/foreach})
</select>
