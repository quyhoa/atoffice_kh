...
({assign var="page_name" value="貸し止め一覧"})
...
<table border="1" bgcolor="#000000" width="500" style="margin:0px auto 30px">
	<tbody>
		...
		<tr>
			<td style="color:#FFFF00">・ ({$error})</td></tr>
	</tbody>
</table>
({/if})
<form id="rental_form" class="form_custom" name="search" method="POST" action="./">
	<input type="hidden" name="m" value="admin">	
	<input type="hidden" name="a" value="page_({$hash_tbl->hash('list_c_customer_stt','page')})"/>
	<input type="hidden" value="0" name="page_num" id="page_num"/>
			
	<ul class="list_check_custom">
		<li><label><input type="checkbox" ({if $check}) checked=checked ({/if}) name ="c1" value="0"/>有効期限なし（無期限）</label></li>
		<li>				
			<input type="checkbox" ({if $dBeginRequests}) checked=checked ({/if}) class="checkbox" name ="c2" value ="1" />
			<label>有効期限： 
				<input name="date1" id="demo1" ({if $ok}) value="({$begin_limit})" ({else}) value ="({$date_current})" ({/if}) type="text" style="width: 85px; padding: 0px 5px">		

				<a ({if $ok})style="cursor: pointer; pointer-events: auto;"({/if}) class="none_decoration disable" href="javascript:NewCal('demo1','YYYYMMDD')">
					<img src="./img/cal.gif" style ="margin: -3px 5px;" width="16" height="16" border="0" alt="Pick a date">
				</a>
			～ 
			    <input name="date2" id="demo2" ({if $ok}) value="({$end_limit})" ({else}) value ="({$date_current})" ({/if})  type="text" style="width: 85px; padding: 0px 5px">
			    <a  ({if $ok})style="cursor: pointer; pointer-events: auto;"({/if}) class="none_decoration disable" href="javascript:NewCal('demo2','YYYYMMDD')">
			   		<img src="./img/cal.gif" style ="margin: -3px 5px;" width="16" height="16" border="0" alt="Pick a date">
			   	</a>
			</label>
		</li>
		<li>
			<input type="checkbox" ({if $dBeginDefault}) checked=checked ({/if}) class="checkbox1" name ="c3" value ="2"/>
			<label>希望利用日：
				<input name="date3" id="demo3" ({if $ok}) value="({$begin_time})" ({else}) value ="({$date_current})" ({/if})  type="text" style="width: 85px; padding: 0px 5px">
				<a ({if $ok})style="cursor: pointer; pointer-events: auto;"({/if}) class="none_decoration disable1" href="javascript:NewCal('demo3','YYYYMMDD')">
					<img src="./img/cal.gif" style ="margin: -3px 5px;" width="16" height="16" border="0" alt="Pick a date">
				</a>
				～ 
				<input name="date4" id="demo4" ({if $ok}) value="({$finish_time})" ({else}) value ="({$date_year_ago})" ({/if})  type="text" style="width: 85px; padding: 0px 5px">
				<a ({if $ok})style="cursor: pointer; pointer-events: auto;"({/if}) class="none_decoration disable1" href="javascript:NewCal('demo4','YYYYMMDD')">
					<img src="./img/cal.gif" style ="margin: -3px 5px;" width="16" height="16" border="0" alt="Pick a date">
				</a>
			</label>
...
({if !$result})	
	結果が出ない
({else})
({if $dBeginDefault}) 希望利用日: ({$dBeginDefault}) ～ ({/if}) ({if $dFinishDefault}) ({$dFinishDefault}) ({/if})<br>
({if $dBeginRequests}) 有効期限: ({$dBeginRequests}) ～ ({/if})  ({if $dFinishRequest}) ({$dFinishRequest}) ({/if})<br>
({if $check})有効期限なし({/if})
...
({foreach from=$result item=item})
	<tr>
		<td align=center>({if $item.flag == 1})有効期限なし({else})有効期限あり({/if})</td>
		<td align=center>({$item.limit_datetime})</td>
		<td align=center>({$item.begin_datetime})</td>
		<td align=center>({$item.hall_name})</td>
		<td align=center>({$item.room_name})</td>
		<td align=center>({$item.time_stop})</td>
		<td align=center>({$item.memo})</td>
		<td align=center>({$item.created_date})</td>
		<td align=center>({$item.admin_name})</td>
		<td align=center><button onclick="view_rental_stop('({$item.stop_user_id})','({$item.stop_id})')">貸し止め情報</button></td>
	</tr>
({/foreach})
</table>
({if $page_num >1})
