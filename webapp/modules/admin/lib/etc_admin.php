...
function admin_fetch_inc_header($display_navi = true, $custom_header = '')
{
    ...
    $_SESSION['_authsession']['username'] = isset($_SESSION['_authsession']['username']) ? $_SESSION['_authsession']['username'] : '';
	$sql = "select name, atoffice_auth_type, hall_id from c_admin_user where username = '".$_SESSION['_authsession']['username']."'";
	$result = db_get_all($sql);
    $v['atoffice_auth_type'] = isset($result[0]['atoffice_auth_type']) ? $result[0]['atoffice_auth_type'] : '';
    ...
    $inc_smarty->assign_by_ref('hash_tbl', new AdminHashTable());
...
{
    if (isset($GLOBALS['AUTH']) && is_callable(array($GLOBALS['AUTH'], 'uid'))) {
