...
function db_admin_insert_c_admin_user($username, $password, $auth_type, $atoffice_auth_type, $full_name, $hall_id)
{
    $data = array(
        'username' => strval($username),
        'password' => md5($password),
        'auth_type' => strval($auth_type),
    	'atoffice_auth_type' => intval($atoffice_auth_type),
    	'name' => strval($full_name),
    	'hall_id' => $hall_id,
    );
    return db_insert('c_admin_user', $data);
}
function db_admin_edit_c_admin_user($id_c_admin_user, $username, $name, $atoffice_auth_type, $hall_name){
     $sql = "UPDATE c_admin_user SET  username = '".$username."', name = '".$name."', atoffice_auth_type = '".$atoffice_auth_type."', hall_id = '".$hall_name."' WHERE c_admin_user_id ='".$id_c_admin_user."'";
     return db_get_all($sql);
}
...
function db_admin_c_member_id_list4cond_pne_point($ids, $cond_list)
{
    ...
    if (isset($cond_list['e_rank']) && $cond_list['e_rank']) {
...
function db_admin_c_member_id_list4cond_mail_address($ids, $cond_list)
{
    ...
    // 携帯メールアドレスの有無で絞る
    if (isset($cond_list['is_ktai_address']) && $cond_list['is_ktai_address'] == 1) {
        $wheres[] = "ktai_address <> ''";
    } elseif (isset($cond_list['is_ktai_address']) && $cond_list['is_ktai_address'] == 2) {
        $wheres[] = "ktai_address = ''";
    }
...
function db_admin_c_member_id_list4cond_c_profile($ids, $cond_list, $type)
{
    ...
    $profile_cond = isset($cond_list['profile']) ? $cond_list['profile'] : null;
...
function _db_admin_c_member_id_list($cond_list, $order = '')
{
    ...
    // ポイントで絞り込み
    if ((isset($cond_list['s_rank']) && $cond_list['s_rank']) || (isset($cond_list['e_rank']) && $cond_list['e_rank'])) {// add by quyhoa
...
function validate_cond($requests)
{
    ...
    $profile_req = isset($requests['profile']) ? $requests['profile'] : null;
...
function cond_name_list($cond_list, $select_last_login)
{
    ...
    if (isset($cond_list['id']) && !is_null($cond_list['id'])) {
        // ID
        $cond_name_list['id']['name'] = 'ID(完全一致)';
        $cond_name_list['id']['value'] = $cond_list['id'];
    }

    if (isset($cond_list['username']) && !is_null($cond_list['username'])) {//add by quyhoa
        // ログインID
        $cond_name_list['username']['name'] = 'ログインID';
        $cond_name_list['username']['value'] = $cond_list['username'];   
    }

    if (isset($cond_list['nickname']) && !is_null($cond_list['nickname'])) {//add by quyhoa
        // ニックネーム
        $cond_name_list['nickname']['name'] = WORD_NICKNAME;
        $cond_name_list['nickname']['value'] = $cond_list['nickname'];
    }

    if (isset($cond_list['last_login']) && !is_null($cond_list['last_login'])){//add by quyhoa
        // 最終ログイン
        $cond_name_list['last_login']['name'] = '最終ログイン';
        $cond_name_list['last_login']['value'] = $select_last_login[$cond_list['last_login']];
    }

    if ( (isset($cond_list['s_year']) && !is_null($cond_list['s_year'])) || (isset($cond_list['e_year']) && !is_null($cond_list['e_year']))) {// add by quyhoa
        // 生年月日
 
        $cond_name_list['year']['name'] = '生年月日';
        if (isset($cond_list['s_year']) && !is_null($cond_list['s_year'] )) {
            $cond_name_list['year']['value'] = $cond_list['s_year'].'年 ～ ';
        } else {
            $cond_name_list['year']['value'] = '開始指定なし ～ ';
        }
        if (isset($cond_list['e_year']) && !is_null($cond_list['e_year'])) {
    ...
    //プロフィール
    $profile_list = db_member_c_profile_list();
    $profile_cond = isset($cond_list['profile']) ? $cond_list['profile'] : null;// add by quyhoa
    ...
    //メールアドレス
    if (isset($cond_list['mail_address']) && !is_null($cond_list['mail_address'])){// add by quyhoa
       $cond_name_list['mail_address']['name'] = 'メールアドレス(完全一致)';
       $cond_name_list['mail_address']['value'] = $cond_list['mail_address'];
    }

    //PCメールアドレスの有無
    if (isset($cond_list['is_pc_address']) && !is_null($cond_list['is_pc_address'])){// add by quyhoa
    ...
    //携帯メールアドレスの有無
    if (isset($cond_list['is_ktai_address']) && !is_null($cond_list['is_ktai_address'])){// add by quyhoa
    ...
    //ポイントランク
    if ((isset($cond_list['s_rank']) && !is_null($cond_list['s_rank'])) || (isset($cond_list['e_rank']) && !is_null($cond_list['e_rank']))) {// add by quyhoa
        ...
        if (isset($cond_list['e_rank']) && !is_null($cond_list['e_rank'] )) {
