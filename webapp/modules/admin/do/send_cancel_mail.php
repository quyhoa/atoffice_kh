...
// 招待メール一括送信
class admin_do_send_cancel_mail extends OpenPNE_Action
{
...
//	var_dump($_REQUEST);
 //       if ($requests['input'] || empty($requests['mails'])) {
	if(isset($_REQUEST['cancel'])){	// send_cancel_mail cancel
/*
...
*/
	}else if(isset($_REQUEST['cancel2'])){	// send_cancel_mail2 cancel
		openpne_forward($module_name, 'page', 'send_cancel_mail');
		exit;
	}else if (!isset($_REQUEST['mails'])) {
            openpne_forward($module_name, 'page', 'send_cancel_mail');
            exit;
	}else if(isset($_REQUEST['submit2'])){	// send_cancel_mail2 excute
		$mail=split(";",$_REQUEST['mails']);
		foreach($mail as $m){
//			echo trim($m)."-/-".$_REQUEST['subject']."-/-".$_REQUEST['message']."<br>";
			if(trim($m)=="") continue;
			put_mail_queue(trim($m), $_REQUEST['subject'], $_REQUEST['message']);
//			echo "[".$res."]</br>";
		}
		...
		//page ある
		//reserve_id 名前の変換
		$_POST['reserve_id']=$_POST['reserveid'];

		openpne_forward($module_name, 'do', 'reserve_cancel');
		exit;
	}
...
*/
        openpne_forward($module_name, 'page', 'send_cancel_mail_confirm');
        exit;
    }
}
